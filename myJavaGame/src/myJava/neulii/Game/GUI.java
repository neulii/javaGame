...
public class GUI implements GameObject{
	...
	public GUI(Game game, MaterialManager mm) {
		this.game = game;
		this.mm = mm;
		//testcomment
	...
	public void render(Graphics g) {
		...
		if(debugViewActive) {
			...
			Tile tempField = game.getMap().getTileFromCoordinate(game.getMousePos().x,game.getMousePos().y);
			...
			if(tempField!=null) {
				...
				showCoordinate = game.getMap().getMapCoordinateFromTile(tempField).x + " / " + game.getMap().getMapCoordinateFromTile(tempField).y;
			}
			else {
				showResource = "0";
				fieldType = "none";
			...
			g.drawString(infoString1 ,(float) (game.getGameWindowDimension().getWidth()-200),20);
			g.drawString(infoString2 ,(float) (game.getGameWindowDimension().getWidth()-200),40);
			g.drawString(infoString3, (float) (game.getGameWindowDimension().getWidth()-200), 60);
		}
		...
		if(moneyWarning) {
			...
			int y = game.getMousePos().y+40;
			
			//when mouse is in the bottom of gamewindow
			if(y+30>game.getGameWindowDimension().getHeight()) {
				y=(int) (game.getGameWindowDimension().getHeight()-30);
			}
			else {
				y = game.getMousePos().y+40;
			}
	...
	public void update(long dT) {
		...
		for (ProductionTile tile : menuTiles) {
			if(tile.isInside(game.getMousePos())) {
			...
			if(allOver) {
				game.setMouseOverMenu(true);
			}
			else {
				game.setMouseOverMenu(false);
				hooveredTile = null;
			}
		}
	}
