...
public class InputListener extends MouseAdapter implements MouseInputListener, KeyListener{
	...
	private Image changeingTile = null;
	private Game game;
	private GUI gui;
	...
	public InputListener(Game game, GUI gui) {
		this.game = game;
		this.gui = gui;
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		Tile tile = game.getMap().getTileFromCoordinate(e.getX(), e.getY());		
	
		//left Mousebutton
		if(e.getButton()==MouseEvent.BUTTON1) {
			
			//when mouse is over menu
			if(game.getMouseOverMenu()) {
			...
			if(!game.getMouseOverMenu()) {
				...
				if(gui.getElementMarked()!=null) {
					...
					if(canBuildOnActualField && (gui.getElementMarked().getCostOfTile()<=game.getMaterialManager().getMoney())) {
						...
						if( gui.getElementMarked() instanceof Production_Furnace) {
							newTile = new Production_Furnace(tile,gui.getElementMarked().getFieldType(),game.getMaterialManager());
						}
						else
							newTile = new ProductionTile(tile,gui.getElementMarked().getFieldType(),game.getMaterialManager());
						...
						game.getMap().changeField(tile, newTile);
						
//						tile.setImage(gui.getElementMarked().getImage());				
//						tile.setFieldType(gui.getElementMarked().getFieldType());
//						
						game.getMaterialManager().subMoney(((ProductionTile)newTile).getCostOfTile());
	...
	public void mouseMoved(MouseEvent e) {
		...
		if(!game.getMouseOverMenu()) {
			...
			activeTile = newTile;
			newTile = game.getMap().getTileFromCoordinate(e.getX(), e.getY());
			...
			if(newTile!=activeTile) {
				...
				if(gui.getElementMarked()!=null) {
					...
					if(gui.getElementMarked().getBuildOn(newTile.getFieldType()) ) {
						...
						//if enough money
						if(gui.getElementMarked().getCostOfTile()<=game.getMaterialManager().getMoney()) {
	...
	public void keyPressed(KeyEvent e) {
		...
		if(e.getKeyCode()==KeyEvent.VK_B) {
			...
			MarketWindow marketWindow = new MarketWindow(game.getMaterialManager());
			marketWindow.show();
		...
		if(e.getKeyCode()==KeyEvent.VK_ENTER) {
			cheatInput = JOptionPane.showInputDialog(null, "Cheat-Code eingeben:", "Cheat Konsole", JOptionPane.INFORMATION_MESSAGE);
			CheatAction.doCheat(game, cheatInput);
		}
		
		if(e.getKeyCode()==KeyEvent.VK_RIGHT || e.getKeyCode()==KeyEvent.VK_D) {
			game.getMap().moveRight(true);
		}
		
		if(e.getKeyCode()==KeyEvent.VK_LEFT || e.getKeyCode()==KeyEvent.VK_A) {
			game.getMap().moveLeft(true);
		}
		
		if(e.getKeyCode()==KeyEvent.VK_UP || e.getKeyCode()==KeyEvent.VK_W) {
			game.getMap().moveUp(true);
		}
		
		if(e.getKeyCode()==KeyEvent.VK_DOWN || e.getKeyCode()==KeyEvent.VK_S) {
			game.getMap().moveDown(true);
		}
	}
	...
	public void keyReleased(KeyEvent e) {
		...
		if(e.getKeyCode()==KeyEvent.VK_RIGHT || e.getKeyCode()==KeyEvent.VK_D) {			
			game.getMap().moveRight(false);
		}
		
		if(e.getKeyCode()==KeyEvent.VK_LEFT || e.getKeyCode()==KeyEvent.VK_A) {
			game.getMap().moveLeft(false);
		}
	
		if(e.getKeyCode()==KeyEvent.VK_UP || e.getKeyCode()==KeyEvent.VK_W) {
			game.getMap().moveUp(false);
		}
		
		if(e.getKeyCode()==KeyEvent.VK_DOWN || e.getKeyCode()==KeyEvent.VK_S) {
			game.getMap().moveDown(false);
		}
	}
}